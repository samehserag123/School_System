...
class DailyClosure(models.Model):
    ...
    def save(self, *args, **kwargs):
        # 1. حساب الفارق تلقائياً قبل الحفظ
        self.variance = self.actual_cash - self.total_cash
        
        with transaction.atomic():
            super().save(*args, **kwargs)
            
            # 2. ربط جميع الإيصالات غير المغلقة بهذا الجرد وتغيير حالتها
            from .models import Payment
            unclosed_payments = Payment.objects.filter(is_closed=False)
            
            # تحديث حالة الإيصالات لتصبح "مغلقة" وربطها بهذا الجرد
            unclosed_payments.update(is_closed=True, closure=self)
            
            print(f"DEBUG: تم إغلاق {unclosed_payments.count()} إيصال للجرد رقم {self.closure_id}")
