...
def sync_payment_to_treasury(sender, instance, created, **kwargs):
    if created:
        # 1. تحديد رقم الإيصال المرجعي (هذا الحقل يجب أن نعتمد عليه دائماً لأنه سريع)
        if instance.receipt_number:
            receipt_ref = str(instance.receipt_number)
        else:
            # دمجنا الـ ID لتأكيد عدم التكرار لنفس العملية
            receipt_ref = f"REC-{instance.id}-{uuid.uuid4().hex[:4]}"
        ...
        # 🟢 3. الفحص التلقائي: هل هذه الحركة عبارة عن خصم؟
        is_discount_tx = "خصم" in category_name or "كوبون" in category_name

        # 🟢 4. فحص التكرار السريع (نستخدم receipt_number فقط لأنه فريد ومفهرس)
        ...
        if not exists:
            GeneralLedger.objects.create(
                ...
                is_discount=is_discount_tx,  # 🟢 تمرير العلامة لإنقاذ سرعة الخزينة لاحقاً
